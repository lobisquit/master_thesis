...
While the last one is a bare Steiner-tree problem, the first and the second
iteration required different values for the problem parameters, see
autoref:quantities_constraints for further details.

#+NAME: optimization_params
#+CAPTION: Values for problem parameters in the first two iterations.
#+ATTR_LATEX: :align crr
| Parameters     | [[ac:dslam][DSLAM]] sub-problem | Routers sub-problem |
|----------------+-------------------+---------------------|
| $n_M$ [unit]   |                48 |                 400 |
| $d_M$ [m]      |              1500 |                   - |
...
In autoref:optimization_params it can be seen that the cost per unit of sub-root
node, $c_r$, is splitted in two numbers: they are the cost of one physical
device and the lump sum for its connection to the mainframe. \\
The price and the number of ports are given by the most popular items available
in the market and what appears to be the industry standards. cite:CiscoWAN
...
exceeded all resources available for our instances. Although not conclusive, the
solver provided useful insights on the optimal solution, specifically a lower
bound for the objective function.

This value was then compared to the cost of the heuristic solution,
whose algorithm is explained in autoref:methodology-heuristic.

autoref:solution-table clearly shows that the sub-optimal heuristic approach is
indeed remarkably close to the theoretical optimum, proving that the choices and
approximations made indeed captured the relevant features of the problem.
...
| [[ac:ilp][ILP]] bound [€]      |    10 |    1800 |
| Heuristic cost [€] |  1000 |    2000 |
| Heuristic gap      | 9900% |     11% |
#+TBLFM: @5$2='(format "%d%% "(truncate (* 100 (/ (float (- @4$2 @3$2)) @3$2))));N::@5$3='(format "%d%% "(truncate (* 100 (/ (float (- @4$3 @3$3)) @3$3))));N

A visual representation of the obtained clusters in the two steps is given in TODO cit,
where again the map is cropped in order to scale at the proper level of detail.

#+LABEL: fig:heuristic-DSLAM
#+ATTR_LATEX: :width 4in
#+CAPTION: City topology is converted into an abstract graph.
[[file:~/Archivi/tesi/figures/heuristic-DSLAM.png]]

#+LABEL: fig:aachen_city_graph
#+ATTR_LATEX: :width 4in
#+CAPTION: City topology is converted into an abstract graph.
[[file:~/Archivi/tesi/figures/heuristic-2router.png]]

#+LABEL: fig:ILP-DSLAM
#+ATTR_LATEX: :width 4in
#+CAPTION: City topology is converted into an abstract graph.
[[file:~/Archivi/tesi/figures/ILP-DSLAM.png]]

#+LABEL: fig:aachen_city_graph
#+ATTR_LATEX: :width 4in
#+CAPTION: City topology is converted into an abstract graph.
[[file:~/Archivi/tesi/figures/ILP-2router.png]]
