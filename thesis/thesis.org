...
This section will discuss which theoretical framework and tools have been used
throughout the analysis and the motivation behind them.

As presented before, this thesis is divided into two main tasks: first the
design part, where a plausible network for the city of Aachen is devised, and
then the optimization part, where a self learning algorithm is written in
order to tune the system.
...
Unfortunately, schematics for Aachen access network are not publicly available,
...
hopefully achieve decent results in a more reasonable amount of time.
...
In [[autoref:results-network_design]] all this approximations, that are required for the algorithm
to be fast enough for our case, will prove to be good ones, i.e. to be close to the
theoretical optimum.
...
In order to obtain a decent map, the two OpenStreetMap datasets corresponding to
buildings and streets of the entire state of North Rein-Westphalia were
downloaded, merged and cropped to remove anothing unwanted, i.e. outside the
border of Aachen. cite:OpenStreetMap

After those operations, the remaining 58.305 constructions and 9.759 roads,
paired with the population density information, were employed to draw the map in
...
As can be seen in the plot, the data is indeed quite accurate and suitable for
the conversion to an abstract graph with streets as edges and road crossing as
vertices. \\
This procedure was performed using a dedicated library that took care of all the
quirks of OpenStreetMap measures[fn:2]: for example a tolerance was added in
order to consider two geographical polygons connected.

After that, all nodes that were too close to each other less than 20m were
merged: this lowered the number of variables in the upcoming analysis and
removed many useless details.

Then buildings were assigned residents, users of our access network, uniformly
with respect to the area population density and building surface, as was
explained in autoref:methodology-geographical_analysis.

To integrate customers information into the road graph, a first attempt was made
where new vertices were created for each building, but the number of variables
turned out to be too high: a decision was then made to assign the inhabitants of
the city to the closer road crossing. \\
...
At the end of this pre-processing phase, the final graph has 7.231 vertices and
9.272 edges and its complexity can be handled in the following analysis. \\
[[autoref:fig:aachen_city_graph]] shows the final result in a central area of the
Aachen district, since the elements of obtained graph are too close to each
other to be discerned at the previous level of zoom.
...
performed starting from the edge of the network, first positioning the [[ac:dslam][DSLAM]],
then the main routers and finally the mainframe.

While the mathematical formulation was the same each iteration required
different values for the problem parameters, listed in
autoref:optimization_params. Please refer to autoref:quantities_constraints for
further details about each variable meaning.
...
The cost per unit of sub-root node, $c_r$, is split in two numbers: they are
the cost of one physical device and the lump sum for its connection to the
mainframe respectively. \\
The price and the number of ports are given by the most popular items available
in the market and what appears to be the industry best practices. cite:CiscoWAN

Some values were omitted from the table when not relevant in the specific
iteration: for example the unique mainframe is a fixed cost for the last step
and so can be ignored.

As anticipated in autoref:methodology-network_design the exact solution for any
of these two problems could not be obtained using [[ac:ilp][ILP]]. Even using a commercial
software such as CPLEX cite:Cplex, in fact, computational time and memory demand
exceeded all resources available.

Although not conclusive, the solver provided useful insights on the optimal
solution, specifically a lower bound for the objective function. These limits
were then compared to the cost of the heuristic solution, whose algorithm is
explained in autoref:methodology-heuristic. [[autoref:solution_table]] clearly shows
that the sub-optimal heuristic configuration is indeed remarkably close to the
theoretical optimum, proving that the choices and approximations made previously
indeed captured all relevant features of the problem.

#+NAME: solution_table
#+CAPTION: Cost of heuristic solution is compared to the theoretical limit given by [[ac:ilp][ILP]].
#+ATTR_LATEX: :align crr
| Problem            | [[ac:dslam][DSLAM]] | Routers |
|--------------------+-------+---------|
| Number of clusters |  1115 |      70 |
| [[ac:ilp][ILP]] bound [€]      |   990 |    1800 |
| Heuristic cost [€] |  1000 |    2000 |
| Heuristic gap      |    1% |     11% |
#+TBLFM: @5$2='(format "%d%% "(truncate (* 100 (/ (float (- @4$2 @3$2)) @3$2))));N::@5$3='(format "%d%% "(truncate (* 100 (/ (float (- @4$3 @3$3)) @3$3))));N

A visual representation of the obtained clusters is given
in [[autoref:fig:heuristic_result]], where again the map is cropped in order to
scale at the proper level of detail, and in autoref:fig:heuristic_mainframe.
...
\begin{figure}[htp]
  ...
  \end{subfigure}
  \caption{The sub-roots nodes in red are hubs for terminals, black points.}
...
#+ATTR_LATEX: :height 4.5in
#+CAPTION: The mainframe, red dot, is located in an industrial complex and is connected to all routers.
