...
user. cite:Andrews1998 Moreover, they consider a cost for edge activation
proportional to the bandwidth used or an upfront cost, accounting for leasing of
existent infrastructure of its physical realization.
...
spread across 160.85km^2. So, new tools and formulations need to be devised in
order to properly solve the problem and possibly achieve better bounds.

# SDN: general concept + small history

# flow control
Given our reference scenario, the possibilities offered by an [[acp:sdn][SDN]] approach are
limited. Since topology is rigidly hierarchical no routing is possible as
usually, to lower the capital expenditure, no redundant paths are added to
common customers.
...
amount network resources to each user, depending on its application
requirements. For example, video streaming shows a different profile than
traditional web browsing and this needs to be taken into account when
administrating the infrastructure.
...
Once individual metrics are evaluated, it is still not clear how to aggregate
the single [[ac:qoe][QoE]] into a global score, that is meaningful for ... .
...
competition between each user that wants in fact to maximize its own
\emph{utility}. It is the main task of the network administrator to guide
individual demands to a stable and \emph{fair} operation point. ``Fairness'' is
indeed an ill-defined concept, that can be seen here as a condition all users
can connect to the network without being overly penalized.

Pareto-optimal points are suggested by Douligeris \etal to capture this elusive
concept, as no user can increment its own utility without worsening it at least
for one of its fellows. cite:Douligeris1987 No criterion is however given in
order to choose among the possibly infinite number of these desirable solutions.

Mazumdar \etal eventually proposes the so-called \emph{Nash arbitration scheme},
a stable operation point, located in the throughput space, of the cooperative
game played among users. cite:Mazumdar1991 Such point is proved to exist and be
the only one satisfying symmetry, independence of irrelevant alternatives,
Pareto optimality, when user [[acp:qoe][QoE]] are maximized.

Moreover, they characterize this \emph{scheme} as the point that maximize the
product of individual user utilities, giving a straightforward way to compute it
in practice.

It is important to discriminate between the Nash arbitration scheme from the
popular Nash equilibrium, as the former arises from a \emph{cooperative} game,
while the latter from a \emhp{competitive} one. Such equilibria are situations
where no user can increment its own utility, by changing its action, while
others behaviour stays the same. In our context, these points are not Pareto
optimal and therefore not interesting for out purposes. cite:Mazumdar1991
