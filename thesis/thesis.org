...
\begin{titlepage}
  % add space for paperback
  \iftoggle{images_titlepage}{ }{ \vspace*{1.5in} }
...
This chapter will present which theoretical tools and frameworks are employed in
our analysis.

First, in autoref:methodology-geographical_analysis, we show how relevant
information about city topology is extracted from publicly available data,
namely population density, streets and buildings position.

Then in autoref:methodology-network_design, we will show how to design a network
given valid paths across the city area and supposed user demand. As mentioned
earlier this will be performed solving an optimization problem, akin to Steiner
tree.

Finally, obtained network is optimized in
autoref:methodology-network_optimization with respect to user requirements and
applications, distributing available network resources in a \emph{fair} way.
Global user utility will be maximized using a game-theoretical framework, called
\emph{Nash arbitration scheme}.
...
All relevant parameters have been collected in [[autoref:tab:quantities_constraints]] and
will be taken for granted from now on.

#+NAME: tab:quantities_constraints
...
the users respectively, as shown in [[autoref:fig:video_quality_report]] cite:GoogleVideoQualityReport.

#+NAME: fig:video_quality_report
...
#+RESULTS[2ceec3ea1f33a4389166ece8a602f2c7dfe13f21]: fig:video_quality_report
...
#+CAPTION: Utilities vs available bandwidth for a given service.
...
The best solution of this problem is proven to be the so-called
...
Moreover, it can be found simply maximizing the product $f$ of all the
utilities: this gives us a straightforward criterion to rank all possible
network configurations. We then define
...
\begin{bigalgorithm}
  ...
  \begin{algorithmic}
    ...
    \EndIf
    \If{ Objective $f$ improvement is negligible in the last L iterations}
...
This chapter shows what we obtain applying the tools presented in
autoref:methodology in our specific context, the city of Aachen.

As its theoretical counterpart, here we first extract relevant geographical
features from publicly available maps, in autoref:results-geographical_analysis.

Then, in autoref:results-network_design, once the abstract city topology is
known, a network is designed on top of it, solving an optimization problem both
using an exact [[ac:ilp][ILP]] solver and an heuristic approach.

Obtained network is finally optimized in autoref:results-network_optimization,
such that each user enjoys a \emph{fair} amount of network resources, according
to the \emph{Nash arbitration scheme}. This optimal operation point is then
compared with legacy strategies, namely \emph{proportional fairness}, in order
to assess whether this new proposed technique improvement is significant or not.
...
#+RESULTS[ec9308145dc39ddbf96780a266a77c5986da0d71]: obj_vs_p_nothing0.5
...
#+RESULTS[4a412bd1c0690c25d8f1f5e6c997c15089d6dfab]: obj_vs_p_nothing0.9
