...
#+RESULTS[b87efdfcd70ed6f32f723d0cbadcc0a77b2cd8ab]: fig:simulator_downlink
...
#+RESULTS[1cf142fbce7974ddda86f326c04f993d5aad73f5]: fig:simulator_uplink
...
#+BEGIN_SRC asymptote :file ../figures/tbf_diagram.pdf
  ...
  draw((-2*r, r) -- (r, r),
       arrow=Arrow,
       L=Label("Input traffic", align=N));

  real arrow_l = r;
  draw((x + 2*r, 3*r + tq_height + arrow_l) -- (x + 2*r, 3*r + tq_height - tq_padding),
       arrow=Arrow,
       L=Label("Token generation", align=N, position=BeginPoint));
  ...
  for (int n = 0; n < n_tokens; ++n) {
    ...
    if (n < packet_sizes[0]) {
      fill(box((x + r, y), (x + 3*r, y + token_size)), highlighter);
    }
    draw(box((x + r, y), (x + 3*r, y + token_size)));
  }
  ...
  for (int n = 0; n < packet_sizes.length; ++n) {
    ...
    if (n == 0) {
      fill(box((x1, 0), (x2, 2*r)), highlighter);
    }
    draw(box((x1, 0), (x2, 2*r)));
  }
#+END_SRC

#+RESULTS[b93901e5097f793991de87557649d02f7ef63087]: fig:tbf_diagram
[[file:../figures/tbf_diagram.pdf]]

*** Optimization approach
#+NAME: utility_example
#+BEGIN_SRC asymptote :file ../figures/utility_example.pdf
  settings.outformat="pdf";
  texpreamble("\usepackage[charter]{mathdesign}");
...
#+RESULTS[25a3522030b79e80661ed13e6cb805ac76932be5]: utility_example
