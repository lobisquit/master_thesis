...
bandwidth. cite:Georgopoulos2013,Schatz2011 These studies highlight a link
between [[ac:qoe][QoE]] and this well defined network metric, described by
autoref:eq:utility_general.
...
service quality perception, that varies with the application.
...
optimal solution of this problem was proven to be the so-called \emph{Nash
...
to rank all possible network configurations. Just to avoid issues related to CPU
floating point precision, a different but equivalent formulation is evaluated in
simulations.
...
More formally, the fair working point is identified as the Nash arbitration
scheme $\vec{\rho}$, solution of the following mathematical problem.
...
algorithm, provided in autoref:lst:alg:heuristic_flow.

These strategies will be tested under different network conditions, specifically
lower or higher offered traffic or different user profiles. The former property
will be described in our simulations via $p_{active}$ parameter, that will be
the probability a certain user is communicating or not. In order to account for
the latter factor, applications are randomly split into video streaming and web
page browsing, according to probability $p_{streaming}$.

Requested videos can be either Low Definition (360p), Medium Definition (720p),
or High Definition (1080p). According to \emph{Google Video Quality Report},
each one of those is experienced by 5%, 10% and 85% of the users respectively.
...
Each type of traffic is assigned a utility function, as shown in
autoref:fig:utilities, that links user perception to available resources. As
mentioned in autoref:methodology-qoe, these relationships were measured
empirically polling a number of users enjoying different bandwidth levels.
...
#+RESULTS[9bd307d37033151a56e0501c609a108a2d4b76f1]: fig:obj_vs_p_nothing0.1
...
#+RESULTS[bc539a96db2598fbafa24ecc510a5c21189d902c]: fig:obj_vs_p_nothing0.5
...
#+RESULTS[878fd4cbd050284034b884c07c52e47f4232db3a]: fig:obj_vs_p_nothing0.9
