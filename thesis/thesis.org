...
cite:Georgopoulos2013,Schatz2011 Their shape is shown in fig...

# TODO set fig, redo plot with new values

#+NAME: utility_example
#+BEGIN_SRC asymptote :file ../figures/utility_example.pdf :noweb yes :tangle ciao.asy
  <<asymptote_preamble>>
  ...
  draw(graph(f2, xmin, xcenter), dashed + grey, legend=Label("Ideal utility", fontsize(10)));
  draw(graph(f1, xmin, xmax), legend=Label("Smoothed utility", fontsize(10)));
  ...
  attach(legend(p=invisible, linelength=1cm, xmargin=0), point(E), NE);
#+END_SRC

#+CAPTION: A smoothed version of the shifted unit step is employed, to guide the optimization.
#+ATTR_LATEX: :height 7cm
#+LABEL: utility_example
#+RESULTS[52f6ec3f067dd8083c848821ccab803bc0174838]: utility_example
...
Such point is unique and pareto-optimal, and it can be found maximizing the
...
This has proven to be a reliable way to distribute network resources, but it
does not take into account user perception of the services that can vary
according to the application, see TODO fig utilities.
