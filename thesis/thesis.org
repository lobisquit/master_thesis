...
As mentioned before, we will analyze the behaviour of the network for different
bandwidth demands and application profiles, in order to assess weather a central
controller can actually improve performance for its users.

Former point is described in our simulations via $p_{active}$ parameter, that is
the probability a certain user is communicating or not. In order to take of the
latter factor, instead, applications are randomly split into video streaming and
web page browsing, according to probability $p_{streaming}$.

*** objective function vs p_active                                 :ignore:
