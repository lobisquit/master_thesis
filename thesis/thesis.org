...
optimization problem using residential buildings and population distribution
across the area.

The network was then run in a custom simulator, both with and without a
controller, in order to assess its contribution on user perceived [[ac:qoe][QoE]].

An operation point the controller is requested to achieve that maximize
# fairness
# better than legacy
...
METHODOLOGY This section will discuss which theoretical framework and tools that
have been used throughout the analysis and the motivation behind them.
...
RESULTS This chapter follows the what was presented in [[autoref:methodology]] and
shows what can be obtained using those theoretical frameworks and tools.
...
Once extracted the relevant feature of the geographical area, the design of such
a network consists in connecting all terminals to a central unit, aggregating
all user traffic.

# Steiner tree = most general problem

This is equivalent to solve the well-known Steiner tree problem on a graph
$G=(V,\,E)$, where a subset $T$ of nodes in $V$ have to be reached from a root
$r \in V$, choosing some edges in $E$ at lower possible cost. This problem is
proved to be NP-hard and thus has been extensively studied in literature, both
with exact and approximated solutions.
cite:Voss1992,Rehfeldt2015,Koch1998,Leitner2014

# TODO Steiner formulation + brief explanation

\smallbreak

# Andrews

This formulation was extended by Andrews \etal, as they consider a number of
link types with different capacity in order to deliver requested amount to the
user. cite:Andrews1998 Moreover, they assign a cost for edge activation
proportional to the bandwidth used or an upfront cost, accounting for leasing of
existent infrastructure or its physical realization.
...
This approach, though, does not take into account the technological limitations,
such as a limit for traffic aggregator number of ports and maximum cable length.

\smallbreak

Previous point is addressed in Mitcsenkov \etal, where an access network is
built from reference topographic scenarios, such as suburban, town or city, in
increasing population density. cite:Mitcsenkov2011 Moreover, they consider for
the same setting various technologies, such as [[acp:pon][PON]], Active Ethernet and VDSL,
each one with its own constraints.
...
Both are run on reference maps, whose ranging from surfaces of 0.5km^2 to
4.7km^2 and from 400 to 20.000 number of subscribers. These trials show the
viability of the approach, as the approximated solution is within 10% to 15%
the theoretical bound provided via an [[ac:ilp][ILP]] solver, while being still fast to
compute.

\smallbreak

These studies are useful insight about the task we are trying to perform, the
access network design for a city. Proposed tools are not enough, though, as
either the conditions or the scale of our reference scenario are out of reach
for them. Just to mention, Aachen has an estimated number of 40.000 subscribers
spread across 160.85km^2. So, new tools and formulations will be devised in
autoref:methodology-network_design, in order to properly solve the problem and
possibly get closer to the bounds.

# SDN: general concept + small history


# flow control
Given our reference scenario, the levers offered by an [[acp:sdn][SDN]] approach are limited.
As topology is a rigid and hierarchical tree, no routing is possible as, to
lower the capital expenditure, no redundant paths are usually added to
non-commercial customers.

The only tool left on the table is then \emph{flow control}, which is the
ability to give or revoke priority from a certain stream of packets at switching
units.

Traditionally this technique has been employed for congestion avoidance, primary
example being [[ac:tcp][TCP]] strategies to match the bitrate with the actual link capacity.
cite:Allman2009

Nowadays, flow control is becoming necessary in order to allocate the proper
amount resources to users, depending on application requirements. For example,
video streaming shows a different profile than traditional web browsing and this
needs to be taken into account when administrating the infrastructure.

\smallbreak

The link between actual connection metrics to user perceived quality of the
service, necessary to perform a decent resource distribution, has been studied
extensively in literature. cite:Georgopoulos2013,Laghari2012,Reichl2011

These research groups estimated [[ac:qoe][QoE]] via the so-called [[ac:mos][MOS]], the average
subjective evaluation of the service, under a wide spectrum of network
conditions. Georgopoulos \etal, Laghari \etal focus on the actual measurement
and try to fit what found with a shifted power function.
cite:Georgopoulos2013,Laghari2012 Reichl \etal, instead, go for a different
approach, as they \emph{a-priori} suppose a certain shape of the [[ac:qoe][QoE]] logarithmic
function and then explain the data with it. This hypothesis is taken in analogy
to the Weber-Fechner Law, a key principle in psycho-physics connecting the
magnitude of a physical \emph{stimulus} to its perceived intensity.

\smallbreak

Once individual metrics are evaluated, it is necessary to aggregate the single
[[ac:qoe][QoE]] into a global score, in order to choose the best among all possible network
configurations.
...
competition between each user wanting to maximize its own \emph{utility}. The
main task of the administrator is then to guide individual demands to a stable
and \emph{fair} operation point.

``Fairness'' is, from a mathematical point of view, an elusive concept, but it
can be seen here as a condition where all users can enjoy the service without
being overly penalized.

Pareto-optimal points are suggested by Douligeris \etal to capture this
requirements, as no user can increment its own utility without worsening it at
least for one of its fellows. cite:Douligeris1987 No criterion is however
provided in order to choose among the possibly infinite number of these
solutions.
...
only one satisfying symmetry, independence of irrelevant alternatives, Pareto
optimality. Moreover, they characterize this \emph{scheme} as the point that
maximizes the product of individual user utilities, giving a straightforward way
...
relevant for the intended flow control.

An example of this concept is the \emph{prisoner's dilemma}, a non-cooperative
game played between two actors $A$ and $B$.

The two of them is suspect of a crime, but the prosecutor lacks evidence for
this. He has however to convict them for a lesser charge. Both of the criminals,
that have no possible way of communicating each other, can either confess the
main crime or stay silent.
...
the Pareto optimal point, is for the two of them to stay silent. This point,
however, is not a Nash equilibrium, because is of them has incentive, in this
situation, to betray the other and spend no time in prison.

This simple toy example shows how Nash equilibria, originating from competitive
games are often not optimal with respect to the global utility: the operator has
then to enforce a fruitful coordination, the Nash arbitration scheme,
