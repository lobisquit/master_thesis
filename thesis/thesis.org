...
layer of the protocol stack had been static. Each route was either discovered by
the device itself or set by the hand of the administrator: in both cases no
tools were available to quickly modify the behaviour according to changes in
traffic demand or network structure.
...
In order to prevent the ``ossification of the Internet'' cite:Hakiri2014 it is
in fact essential to separate the \emph{data plane}, the actual physical devices
forwarding packets, and the \emph{control plane}, responsible for deciding
routes.

The latter operations are supervised by one or more devices, called
\emph{controllers}, whose task is to orchestrate involved routers and switches
based on current and forecast network status. This moves the routing logic from
the single component to a centralized and fully programmable entity. These
infrastructures are therefore called [[acp:sdn][SDNs]].
...
very interesting in cable networks, where packet gridlock is indeed the main
...
# Currently there is much hype about the OpenFlow standard, an open API that
# enables the central node to control switches and routers. cite:Vaughan2011

Given the current demand for connectivity and bandwidth, [[acp:sdn][SDN]] can be a
ground-breaking approach to further improve the performance using unconventional
approaches. Not only flow control, but also user mobility prediction could be
exploited by the programmable controller logic.
...
this analysis. First, in autoref:methodology-geographical_analysis, we will show
how relevant information from Aachen building and road map can be extracted in
order to design, in autoref:methodology-network_design, a proper access network
based on city topology and population density. Eventually,
autoref:methodology-network_optimization will detail how this infrastructure can
be tuned for maximize user perceived quality of the service.

After applying all these procedures to out case of study, final results are
