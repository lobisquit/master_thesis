...
As shown in these diagrams, each outgoing flow passes through a controller: this
unit controls the rate of the packets before they enter the transmitting
interface, labeled as ``NIC''.

The central network administrator can employ these traffic shapers to give or
revoke priority from a given source, with the ultimate goal of keeping the
network balanced.

In our setting, all data flows are supposed to be [[ac:cbr][CBR]]: the central unit has then
the task of allocating the corresponding bandwidth according to the network
conditions.
...
In order to improve the network service for its users, first we have to define
what we mean for \emph{improvement}. We decided to maximize user [[ac:qoe][QoE]], defined as
a number in $[0,\,1]$, ranging from unusable to perfect link. Each user $i$ is
then assigned a \emph{utility function}, mapping available bandwidth $\rho_i$ to
perceived quality.

These function are taken from literature, and more specifically from works that
study user experience is linked to measurable and objective connection metrics.
cite:Georgopoulos2013,Schatz2011
...
#+BEGIN_SRC python :noweb yes :exports results :results file output
  ...
  profiles = {
      'Streaming HD': (-3.035, -.5061),
      'Streaming MD': (-4.850, -.6470),
      'Streaming LD': (-17.53, -1.048),
      'Web broswing': (-14.98544276, -0.87800541)
  }
  ...
  for profile in profiles:
      y = utility(x, *profiles[profile])
      plt.plot(x, y, label=profile)

  plt.legend()
  plt.title('Utility profiles')
  plt.xlabel('Bandwidth', fontweight='regular')
  plt.ylabel('Utility', fontweight='regular')
...
#+CAPTION: Utilities connect available bandwidth to user satisfaction.
#+ATTR_LATEX: :height 3in
#+LABEL: fig:utilities
#+RESULTS[80ed12479a7eb0a9f5d21e8f3683793b345346ac]: utilities
[[file:../figures/utilities.pdf]]

As shown in autoref:fig:utilities, requested videos can be either Low Definition
(360p), Medium Definition (720p), or High Definition (1080p).

According to \emph{Google Video Quality Report}, these quality rates are
distributed as 5%, 10% and 85% respectively. These proportions will be followed
when simulating the network, in order for the end results to be realistic.
...
#+CAPTION: Distribution of video quality among YouTube users. cite:GoogleVideoQualityReport
...
In order to satisfy all users, we need to reach an operation point for the
network that guarantees optimal and fair flow control.

Each users, in fact, wants to maximize its own utility, but doing so it harms
the [[ac:qos][QoS]] of its fellows. The [[ac:isp][ISP]] has then to act as an arbiter, allocating
available resources in a fair way. The optimal solution of this problem was
proven to be the so-called Nash arbitration scheme of the \emph{game} played
among these actors. cite:Mazumdar1991

Such point is unique and pareto-optimal, and it can be found maximizing $f$,
product of users utilities. This gives us a straightforward criterion in order
to rank all possible network configurations.

In order to avoid issues related to floating point precision in computers, a
different, but equivalent formulation is employed in this thesis.
...
Traditionally, when handling different flows, \emph{proportional fairness} is
applied. According to this rule, each switching unit allocates bandwidth in
\emph{proportion} to user demand.

This has proven to be a reliable way to distribute network resources in the
past, but it does not take into account user perception of the services that can
vary according to the application, see TODO fig utilities.

\smallskip

In autoref:results-network_optimization we will show how these two strategies
rank, in terms of [[ac:qoe][QoE]], in our reference scenario.

*** Flow balancing optimization
From a theoretical point of view, the fair and optimal working point has been
described in the last section as the Nash arbitration scheme $\rho$, solution of
...
\begin{align}
  ...
  \text{given~~}
                 ...
                 & \sum_{i \in DSLAM_j} \rho_i \le \rho_{D,\,MAX} ~ \forall j \in D \label{eq:dslam_max} \\
                 & \sum_{i \in ROUTER_j} \rho_i \le \rho_{R,\,MAX} ~ \forall j \in R \label{eq:router_max}
\end{align}
...
$\rho_{MAX}$ has to be allocated to the users maximising a convex objective
function.

Unfortunately, our case is complicated by the two additional constraints
# autoref:eq:dslam_max and autoref:eq:router_max:
no algorithm is known in literature that is able to solve the problem optimally.
cite:Xing2018

A sub-optimal solution has then to be searched using an heuristic algorithm.
Given the monotonicity of the objective function, the following iterative
...
\begin{bigalgorithm}
  ...
  \begin{algorithmic}
    ...
    \State Initialize all users, each with their utility $u_i$
    ...
    \State Perturb $\rho_k$ of a uniform random quantity in $[0,\, T]$
...
As described in [[autoref:methodology-network_optimization]], we will consider two
possible ways to allocate the total available bandwidth to the access network
users: proportional fairness and the Nash arbitration scheme. While the former
is computed exactly, the latter needs to be approximated via an heuristic
algorithm, see autoref:lst:alg:heuristic_flow.

These strategies will be tested under different conditions the network can find
itself, specifically lower or higher offered traffic or different user profiles.
The former property will be described in our simulations via $p_{active}$
parameter, that will be the probability a certain user is communicating or not.
In order to account for the latter factor, first applications are randomly split
into video streaming and web page browsing, according to probability
$p_{streaming}$.
...
#+CAPTION: Performance for $p_{streaming}$ = 0.1
#+RESULTS[9cb850c13add9a2bb7fdc8c253e5d9e398279275]: fig:obj_vs_p_nothing0.1
[[file:../figures/obj_vs_p_nothing_0.100000.pdf]]


#+NAME: fig:obj_vs_p_nothing0.5
#+BEGIN_SRC R :exports results :results file value :noweb yes
p_streaming = 0.5
...
#+CAPTION: Performance for $p_{streaming}$ = 0.5
#+RESULTS[f3331d15eba07e09b62fe19cb2a15171d2d42f06]: fig:obj_vs_p_nothing0.5
[[file:../figures/obj_vs_p_nothing_0.500000.pdf]]

#+NAME: fig:obj_vs_p_nothing0.9
#+BEGIN_SRC R :exports results :results file value :noweb yes
p_streaming = 0.9
...
#+CAPTION: Performance for $p_{streaming}$ = 0.9
#+RESULTS[26d3e0273f40b13e134a98bf3803cca2ffcf7373]: fig:obj_vs_p_nothing0.9
