...
All relevant parameters for our problem are listed in autoref:quantities_constraints.
...
As ach arc $(i,\,j) \in A$ is assigned a length, in meters, given by the
...
To make notation clearer, we shall introduce the functions $\delta^+, \,
\delta^-: V \rightarrow \mathbb{P}(A)$ associating each node with the outgoing
and ingoing edges respectively.
