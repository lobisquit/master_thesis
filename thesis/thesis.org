...
$r \in V$, choosing some edges in $E$ at lower possible cost. It is proved to be
NP-hard and thus has been extensively studied in literature, both with exact and
approximated solutions. cite:Voss1992,Rehfeldt2015,Koch1998,Leitner2014

A reference representation of the Steiner tree problem is given in
autoref:eq:steiner_tree_reference, where active edges $e \in E$ are marked by a
binary variable $x_e$ and a cost $c_e$, while $\delta^{\pm}(\mathord{\cdot})$
gives the edges entering or exiting its argument, respectively.
...
A mathematical formulation describing the optimal solution given in their paper,
and it is coupled with an approximated solver algorithm, that is obtained via
...
such as the limitations on the number of traffic aggregator ports and cable
length.

\smallbreak

This last point is addressed in Mitcsenkov \etal, where an access network is
...
assumptions. Both are run on reference maps, whose ranging from surfaces of
0.5km^2 to 4.7km^2 and from 400 to 20.000 number of subscribers. These trials
show the viability of the approach, as the approximated solution is within 10%
to 15% the theoretical bound provided via an [[ac:ilp][ILP]] solver, while being still fast
to compute.
...
our reference scenario are out of reach for them. For example, Aachen has an
estimated number of 40.000 subscribers spread across 160.85km^2. So, new tools
and formulations will be devised in autoref:methodology-network_design, in order
to properly solve the problem and possibly get closer to the theoretical bounds.
...
In order to reduce infrastructure cost, our reference topology is a rigid and
hierarchical tree: in this context no routing is therefore possible.
...
amount resources to users depending on their application requirements. For
example, video streaming shows a different profile than traditional web browsing
and this factor needs to be taken into account when administrating the
infrastructure.
...
Once individual metrics are evaluated, it is necessary to aggregate all user [[ac:qoe][QoE]]
...
this requirements, as no user can increment its own utility without worsening it
at least for one of its fellows. cite:Douligeris1987 No criterion is however
...
optimality. Moreover, they prove that this strategy maximizes the product
of individual user utilities, giving a straightforward way to compute it in
practice.
...
minor crimes. Both of the criminals, who have no way of communicating to each
other, can either confess the main crime or stay silent.
...
however, is not a Nash equilibrium, because each one of them has incentive, in
this situation, to betray the other and spend no time in prison.

This simple toy example shows how Nash equilibria, originating from competitive
games are often not optimal with respect to the global utility. In our context,
then, the operator has then to enforce a fruitful coordination, the Nash
arbitration scheme, among players carefully tuning packet flows.
