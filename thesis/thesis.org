...
then the second level routers routers and finally the mainframe.

While the mathematical formulation is the same each iteration requires different
values for the problem parameters, listed in autoref:optimization_params. Please
refer to autoref:quantities_constraints for further details about each variable
meaning.
...
Although not conclusive, the solver provided useful insights on the domain of
solutions, specifically a lower bound for the objective function. These limits
were then compared to the cost of the heuristic solution, whose algorithm is
explained in autoref:methodology-heuristic. \\
[[autoref:solution_table]] clearly shows that the heuristic configuration is indeed
remarkably close to the theoretical optimum, proving that the choices and
