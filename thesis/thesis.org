...
# TODO use greyscale-compatible colormap
...
with the countinous relaxation of problem. 
# TODO ref problem 
These 
...
# TODO: non count, ma Prob. Mass Distribution
...
it is apparent how realizations are overall closer to one, certifying the small,
...
is larger the situation is trickier to interpret: again though proposed solution
tends to move towards the optimal point.

\bigskip

Despite all these accomplishment of proposed heuristic approach, it is currently
only a lower bound on the theoretical optimum, exact solution of the
mathematical problem. Further progress can then be made in the search of the
best working point.
