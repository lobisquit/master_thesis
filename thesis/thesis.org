...
Then all users were assigned to the residential buildings uniformly with respect
to the area population density and building surface, as was explained in
autoref:methodology-geographical_analysis.

A first attempt was made where a new vertex was created for each building but
again the number of variables turned out to be too high: a decision was then
made to assign the inhabitants of the city to the closer road crossing. \\
In order to reduce the introduced approximation error, all long roads were split
and forced to be shorter than 200m: this way the average displacement was
reduced to just 50m, tolerable for our purposes.

At the end of this pre-processing phase, the final graph has 7.231 vertices and
9.272 edges, whose complexity can be handled in the following analysis. \\
Since the elements of obtained graph are too close to each other to be discerned
in the main map [[autoref:fig:aachen_city_graph]] shows just a central area as a
visual reference.
