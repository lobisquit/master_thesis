...
what we mean for \emph{improvement}. We decided to maximize user [[ac:qos][QoS]], defined as
a number in $[0,\,1]$, ranging from unusable to perfect link.

Following what proposed in Cisco Service Level Agreement recommendations, each
type of user application is assigned a certain number of constraints to satisfy
in order for it to work properly. For example, video streaming requires moderate
packet loss and a decent average delay, while the main concern for traditional
web page traffic is the page load time. cite:CiscoSLA
