...
Steiner formulation captures some features of our design goal, but some others
are not taken into consideration, such as technological limitations

Along with all reachability conditions stated in its definition




Many research groups tackled the same problem when the switch from last-mile
copper lines to optical fiber required a complete rebuilding and rethinking of
the infrastructure. Nowadays it is more relevant than ever, as this transition
is taking place in almost all cities and districts, where legacy technology is
getting replaced by future-proof one. cite:Waring1991

Andrews \etal suppose the existence of a certain number of users, each of those
requiring an integral demand $d$ from a central node. cite:Andrews1998

In their model, a distance function $l(\mathord{\cdot})$ among nodes and an
existing core network are given for
