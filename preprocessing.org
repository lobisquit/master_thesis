...
** Interarrival distribution
   ...
   #+BEGIN_SRC bash :tangle scripts/5_distribution.sh :var pcap_files=pcap_files :results none
     ...
     awk_snippet='BEGIN   { min=inf; max=0 }
                       $1 != ($1+0) {} # avoid if non-numeric
                       $1 > max { max=$1 }
                       $1 < min { min=$1 }'
     ...
     do
         ...
         pv $csv_file \
             | gunzip -c \
             | cargo run --release --manifest-path=scripts/interarrival_binner/Cargo.toml $min $max 100 \
                     > ${csv_file}_distribution
     done
