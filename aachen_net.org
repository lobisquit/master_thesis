...
** Project each building on its road
   ...
   TODO polish the graph better
   - FIRST remove edges of length 0: merge the nodes if necessary
   - merge the buildings (even the buildings to the road if necessary): this would lead |V|=16k, |E|=23k
   - recursively prune non terminal "leaves" of the graph
   - merge roads?
   - limit (r, j) arcs number: how?
   - NOTE limit the number of constraints does not seem too effective
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=flatten(utils.org:valid_types) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/08_add_buildings_to_graph.py :var closest_edges_path=flatten(closest_edges_path)
     ...
     with open(graph_path.replace('_raw.json', '_buildings.json'), 'w') as output:
...
* Solve associated ILP problem
  ...
  #+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/12_plot_ILP_result.py
    ...
    # terminals
    nodes_df[(nodes_df["id"] != 'r') & (nodes_df["n_lines"] >= 1)].plot(ax=ax,
                                                                        markersize=10,
                                                                        color='black',
                                                                        zorder=1)
    ...
    if showroot:
        ...
        edges_df[(edges_df['source'] == 'r') & (edges_df['n'] >= 1)].plot(ax=ax,
                                                                          column='n',
                                                                          color="green",
                                                                          linewidth=0.5,
                                                                          zorder=0)
