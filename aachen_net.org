...
* Solve associated ILP problem
  #+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/10_ILP.py
    ...
    G = json_graph.node_link_graph(js_graph).to_directed()
    ...
    for i, (node_id, data) in enumerate(G.nodes(data=True)):
        ...
        out_count_X = sum( [X[node_id][target] for _, target in G.out_edges(node_id)] )

        in_count_N  = sum( [N[source][node_id] for source, _ in G.in_edges(node_id)] )
        out_count_N = sum( [N[node_id][target] for _, target in G.out_edges(node_id)] )
