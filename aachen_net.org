...
#+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/22_output_topology.py
  ...
  # setup n_lines (!=0 only in dlams)
  g.vp['n_lines'] = g.new_vertex_property("int")
  for v in g.vertices():
      g.vp['n_lines'][v] = 0

  for dslam, vertex in dslam_vertex.items():
      buildings_of_dslam = np.array(building_dslam) == dslam
      buildings_ids = list(map(reverse_replacer,
                               np.array(buildings)[buildings_of_dslam]))

      g.vp['n_lines'][vertex] = sum(G_complete.vp['n_lines'].a[buildings_ids])

  assert g.vp['n_lines'].a.sum() == G_complete.vp['n_lines'].a.sum()
