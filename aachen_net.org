...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  # load graph and convert to graph_tool format (Floyd-Warshall is way faster)
  with open(path) as f:
  ...
  # starting point: all terminals are sub-root nodes
  terminals = [v for v in G.vertices() if G.vertex_properties['n_lines'][v] > 0]
  ...
  class Cluster:
      ...
      def __repr__(self):
          return "<Cluster(index={}, elements={})>"\
              .format(self.index, set(self._elements))
  ...
  clusters = { Cluster({ int(t) }) for t in terminals }
  node_cluster_map = {c.index: c for c in clusters}

  min_cluster_distances = vertex_distances.copy()
  ...
  def update_distances(min_cluster_distances, max_cluster_distances, c1, c2):
      for c in clusters:
          if c != c1 and c != c2:
              min_cluster_distances[order(c.index, c_new.index)] = min(
                  min_cluster_distances[order(c, c1)],
                  min_cluster_distances[order(c, c2)]
              )
              max_cluster_distances[order(c.index, c_new.index)] = max(
                  max_cluster_distances[order(c, c1)],
                  max_cluster_distances[order(c, c2)]
              )

  def mergeable(max_cluster_distances, c1, c2, n_M=50, d_M=1500):
      if max_cluster_distances[order(c1.index, c2.index)] <= d_M:
          if c1.n_lines + c2.n_lines <= n_M:
              return True

      return False

  edges = ( order(int(edge.source()), int(edge.target())) for edge in G.edges() )
  ...
  while edges:
      ...
      if mergeable(max_cluster_distances, c1, c2):
          clusters.remove(c1)
          clusters.remove(c2)
