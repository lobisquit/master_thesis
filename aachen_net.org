...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  clusters = vertices[is_terminal]
  cluster_lines = n_lines[is_terminal]
  ...
  # conversion takes some time: use cache file
  ...
  if not distance_path.exists():
      ...
      with h5py.File(distance_path, 'w') as f:
          matrix = f.create_dataset('cluster_dist', (T, T))

          for index, vertex_id in enumerate(clusters):
              if index % 100 == 0:
                  print("Filling distance matrix", index, "/", len(clusters), end='\r')

              matrix[index, :] = np.array(vertex_dist[vertex_id])[is_terminal]

  with h5py.File(distance_path, 'r') as f:
      min_cluster_dist = np.array(f['cluster_dist'])
  ...
  #     is_terminal = np.array([G.vp['n_lines'][v] > 0 for v in G.vertices()])

  #     cluster_ids = - np.ones( (len(vertices), ) )
  ...
  #         nodes_df['cluster'] = cluster_ids

  #         edges_df.plot(color='grey',
  #                       ax=ax)

  #         nodes_df.plot(column='cluster',
  #                       cmap='tab20',
  #                       ax=ax)
  #         plt.draw()

  def objective_function(G, clusters):
      # DSLAMs cost
      sub_root_cost = len(set(clusters)) * params['c_r']

      # fiber cable cost is neglected for now
      # cable_cost = ...
      ...
      for cluster_id in set(clusters):
          cluster_nodes = np.array(list(G.vertices()))[clusters == cluster_id]
          subgraph = GraphView(G, vfilt=lambda v: v in cluster_nodes)

          MST_edges = min_spanning_tree(subgraph)
          total_length += sum(G.ep['length'][e] for e in G.edges() if MST_edges[e])

      excavation_cost = params['c_e'] * total_length

      return sub_root_cost + excavation_cost
  ...
  counter = 0
  while not np.all(np.isnan(min_cluster_dist)):
      ...
      # proceed only if it is not the case
      length = min_cluster_dist[i][j]

      if total_n_lines <= params['n_M'] and \
         joint_diameter <= 2 * params['d_M'] and \
         length <= params['c_r'] / params['c_e']:
          ...
          print("total_n_lines", total_n_lines, 'diam', joint_diameter, 'length', length)
      ...
      if counter % 50 == 0:
          plt.clf()
          plt.hist(cluster_lines, bins=100)
          plt.pause(1)
          plt.draw()
#+END_SRC
