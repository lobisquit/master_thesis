...
#+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/22_output_topology.py
  ...
  old_ids = list(buildings) + dslams + routers + [mainframe_id]
  new_ids = [x + 1 for x in range(len(old_ids))] # reserve 0 as special id
  ...
  # save output result
  with open('data/aachen_net/topology.txt', 'w') as f:
      for child, father in zip(buildings, building_dslam):
          f.write("{},{},{}\n".format(child,
                                      father,
                                      G_complete.vp['n_lines'].a[child]))

      for child, father in zip(dslams, dslam_router):
          f.write("{},{}\n".format(child, father))

      for child, father in zip(routers, router_mainframe):
          f.write("{},{}\n".format(child, father))
#+END_SRC
