...
#+BEGIN_SRC python
  ...
  def ILP_solve(G, params, minimum_gap=1e-4):
      #####################
      # Pre-process graph #
      #####################
      G = G.to_directed()
...
#+BEGIN_SRC python
  def get_terminal_distance(G, is_terminal, distance_path=None, clear_cache=False):
      ...
      if not distance_path.exists() or clear_cache:
          ...
          with h5py.File(distance_path, 'w') as f:
              ...
              terminals = np.array(list(G.vertices()))[is_terminal]
              T = len(terminals)
...
#+BEGIN_SRC python
  ...
  def get_min_path_tree(G, cluster_nodes, params):
      ...
      return best_road_length, best_root, best_tree_edges
#+END_SRC
...
#+BEGIN_SRC python
  ...
  def objective_function(G, clusters, terminals, terminal_dist, terminal_lines, params):
      # DSLAMs cost
      ...
      for index, cluster_id in enumerate(set(clusters)):
          ...
          best_road_length, _, _  = get_min_path_tree(G, cluster_nodes, params)
  ...
  def heuristic_solve(G, params):
      ...
      # measure if the connection is more expensive than a DSLAM: in this cases don't
      # even consider evaluating the merge
      ...
      while True:
          ...
          if n_iter % 50 == 0:
              total_cost = objective_function(G, clusters, terminals, terminal_dist, terminal_lines, params)
              logger.info("Money {:.2f}M€ n_cluster {}".format(total_cost/1e6,
                                                               len(set(clusters))))
...
#+NAME: plot_solution
#+BEGIN_SRC python
  def plot_optimization_result(G, method=None, projection=None, params=None, cut=True):
      ...
      if method == "ILP":
          ...
          r_id = int(find_vertex(G, G.vp['node_name'], 'r')[0])

          edges_df = edges_df[edges_df['gt_source'] != r_id]
          nodes_df = nodes_df[nodes_df['gt_id'] != r_id]

          # find active nodes
          nodes_df['active'] = nodes_df["n_lines"] >= 1
          edges_df['active'] = edges_df['x'] == 1

          # set convenient index
          nodes_df.set_index('gt_id', inplace=True)
          edges_df.set_index(['gt_source', 'gt_target'], inplace=True)

          # mark sub-root nodes
          nodes_df['is_subroot'] = False
          for _, sub_root, edge_idx in G.get_out_edges(r_id):
              # avoid root self-loops
              if sub_root == r_id:
                  continue

              if G.ep['x'].a[edge_idx] == 1:
                  nodes_df.loc[sub_root, 'is_subroot'] = True

      elif method == "heuristic":
          nodes_df.set_index('gt_id', inplace=True)
          edges_df.set_index(['gt_source', 'gt_target'], inplace=True)

          nodes_df['active'] = False
          nodes_df['is_subroot'] = False
          edges_df['active'] = False

          for index, cluster_id in enumerate(set(nodes_df['cluster_id'])):
              print("Loaded cluster", index, "/", len(set(nodes_df['cluster_id'])), end='\r')

              # ignore non-assigned nodes
              if cluster_id == -1:
                  continue

              vertices = nodes_df.index
              cluster_nodes = vertices[nodes_df['cluster_id'] == cluster_id]
              _, root, tree_edges = get_min_path_tree(G, cluster_nodes, params)

              nodes_df.loc[root, 'is_subroot'] = True
              nodes_df.loc[cluster_nodes, 'active'] = True
              edges_df['active'] = np.logical_or(edges_df['active'], tree_edges)
      else:
      ...
      plot_geopandas_graph(nodes_df[nodes_df['active']],
                           ...
                           title="Detail of {} solution".format(method),
                           ax=ax)
...
** DSLAM positioning
$c_r$, cost per DSLAM has to consider also the cable to reach the first level router.
We estimate 1000€ for the DSLAM and 15000€ for the mentioned connection.
...
#+BEGIN_SRC python :noweb yes :var params=extract(DSLAM_params) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/12_solver_ILP_DSLAM.py
  ...
  G_prime = ILP_solve(G, dict(params), 0.2)
...
#+BEGIN_SRC python :noweb yes :var params=extract(DSLAM_params) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/14_plot_ILP_DSLAM.py
  <<imports_&_defaults>>
  <<projection>>
  <<graph_utils>>
  <<plot_solution>>
  ...
  plot_optimization_result(G, method="ILP", projection=projection)
  plt.show()
#+END_SRC
...
#+BEGIN_SRC python :noweb yes :var params=extract(DSLAM_params) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/13_solver_heuristic.py
  <<imports_&_defaults>>
  <<projection>>
  <<graph_utils>>
  <<terminal_dist_cache>>
  ...
  # find optimal configuration
  G_prime = heuristic_solve(G, dict(params))

  # output to graphml file
  G.save(graph_path + "_DSLAM_heuristic.graphml")
#+END_SRC

Plot detail of obtained graph.

#+BEGIN_SRC python :noweb yes :var params=extract(DSLAM_params) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/15_plot_heuristic_DSLAM.py
  <<imports_&_defaults>>
  <<projection>>
  <<graph_utils>>
  <<plot_solution>>
  ...
  plot_optimization_result(G,
                           method="heuristic",
                           projection=projection,
                           params=dict(params), cut=False)
  plt.show()
#+END_SRC
