...
** Create an abstract graph from the roads
   ...
   #+NAME: graph_path
   - data/aachen_net/connected_roads.json
...
** Match each building with the closest road
   ...
   - data/aachen_net/closest_lines.csv

   #+BEGIN_SRC python :noweb yes :tangle scripts/aachen_net/07_get_closest_roads.py :var valid_types=utils.org:valid_types :var closest_lines_path=closest_lines_path :var graph_path=graph_path
     <<all_datasets>>
     ...
     closest_lines                  \
         ...
         .to_csv(org_flatten(closest_lines_path),
...
** Project each building on its road
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=utils.org:valid_types :var graph_path=graph_path :tangle scripts/aachen_net/08_add_buildings_to_graph.py :var closest_lines_path=closest_lines_path
     <<all_datasets>>
