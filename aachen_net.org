...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  # add artificial root node: only supplier!
  R = max(G) + 1

  total_n_lines = sum(data['n_lines'] for _, data in G.nodes(data=True))
  for source, target in G.edges():
      # set only capacity limit at the subroot nodes
      G[source][target]['capacity'] = total_n_lines

  G.add_node(R)
  for node_id in G.nodes():
      G.add_edge(R, node_id, capacity=params['n_M'])

  def get_edge_info(G):
      for source, target, data in G.edges(data=True):
          if source == R or target == R:
              cost = params['c_r']
          else:
              cost = data['length'] * params['c_e']

          yield source, target, data['capacity'], int(cost)
  ...
  start_nodes, end_nodes, capacities, unit_costs = zip(*get_edge_info(G))

  # Define an array of supplies at each node.

  supplies = list(get_node_info(G))
  ...
  print(min_cost_flow.SolveMaxFlowWithMinCost() == min_cost_flow.OPTIMAL)
