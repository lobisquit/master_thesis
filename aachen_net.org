...
** Project each building on its road
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=flatten(utils.org:valid_types) :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/08_add_buildings_to_graph.py :var closest_edges_path=flatten(closest_edges_path)
     ...
     closest_lines = closest_lines[(closest_lines.area > 30) &
                                   (closest_lines.area < 10000)]
     ...
     closest_lines['n_lines'] = closest_lines.population // 4
     closest_lines.loc[closest_lines['n_lines'] == 0.0, 'n_lines'] = 1.0
     ...
     for source, target, data in G.edges(data=True):
         if type(source) == np.int64 or \
            type(target) == np.int64:
             ...
             G.add_edge(int(source), int(target), **data)

     # select buildings stats and output
     buildings_n_lines = [data['n_lines'] for _, data in G.nodes(data=True) if data['n_lines'] > 0]
...
* Solve associated ILP problem
  #+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/10_ILP.py
    ...
    m = Model(log_output=True) # "logs/CPLEX.log")
    ...
    # C)
    for source, target, data in G.edges(data=True):
        obj_func += X[source][target] * data['length'] * c_e
    ...
    for i, (node_id, data) in enumerate(G.nodes(data=True)):
        ...
        if node_id == 'r':
            m.add_constraint(ct=in_count_X == 0,
                             ctname="{} == 0".format(name(node_id, var='in_count_X')))
        elif data['n_lines'] > 0: # terminal
            m.add_constraint(ct=in_count_X == 1,
                             ctname="{} == 1".format(name(node_id, var='in_count_X')))
        else:
            m.add_constraint(ct=in_count_X <= 1,
                             ctname="{} <= 0".format(name(node_id, var='in_count_X')))
        ...
        if node_id == 'r':
            m.add_constraint(ct=out_count_X >= 1,
                             ctname="{} >= 0".format(name(node_id, var='out_count_X')))

        # 4)
        m.add_constraint(ct=D[node_id] <= in_count_X * d_M,
    ...
    for i, (source, target, data) in enumerate(G.edges(data=True)):
        ...
        m.add_constraint(ct=D[target] - D[source] >= edge_length * X[source][target] - d_M * (1 - X[source][target]),
        ...
        # 6)
        # m.add_constraint(ct=N[source][target] <= X[source][target] * n_M,
        #                  ctname="{}".format(name(source, target, var='n_lines_upper')))
