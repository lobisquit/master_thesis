...
* Solve associated ILP problem
  #+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/10_ILP.py
    ...
    G = nx.read_gml(graph_path.replace('_raw.json', '_complete.gml')).to_directed()
    ...
    for source, target in G.edges:
        ...
        X[source][target] = m.binary_var(name(source, target, var='x'))
        N[source][target] = m.integer_var(name(source, target, var='n'))

    D = {}
    for node_id, data in G.nodes(data=True):
        ## set distance counter
        D[node_id] = m.continuous_var(name="d_{}".format(node_id))
    ...
    for node_id, data in G.nodes(data=True):
        in_count_X  = sum( [X[source][node_id] for source, _ in G.in_edges(node_id)] )
        out_count_X = sum( [X[source][node_id] for source, _ in G.out_edges(node_id)] )

        in_count_N  = sum( [N[source][node_id] for source, _ in G.in_edges(node_id)] )
        out_count_N = sum( [N[source][node_id] for source, _ in G.out_edges(node_id)] )
        ...
        # 3)
        if node_id == 'r':
            m.add_constraint(out_count_X > 0)

        # 4)
        m.add_constraint(D[source][node_id] <= in_count_X * d_M)
    ...
    for source, target, data in G.edges(data=True):
        ...
        # 5)
        m.add_constraint(
            D[target] - D[source] >= edge_length * X[source][target] - d_M * (1 - X[source][target])
        )
