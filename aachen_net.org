...
#+BEGIN_SRC python
  def get_vertices_distance(G, cache_path=None, clear_cache=False):
...
#+BEGIN_SRC python
  ...
  def compute_path(G, i, j):
      ...
      mask[ids] = True
      length = G.ep['length'].a.dot(mask)

      paths[i, j] = (mask, length)

      return paths[i, j]

  def get_min_path_tree(G, cluster_nodes, is_terminal, params):
      ...
      for root in cluster_nodes:
          ...
          # tree from root has to reach all terminals (not all nodes in general)
          for node in cluster_nodes:
              if is_terminal[node]:
                  path_edges, path_length = compute_path(G,
                                                         G.vertex(root),
                                                         G.vertex(node))

                  # a cable has to be placed per each customer
                  current_cable_length += path_length * G.vp['n_lines'][G.vertex(node)]
          ...
          if np.all(current_road_lengths) < params['d_M'] and \
             current_road_length < best_road_length:
              ...
              best_cable_length = current_cable_length
              best_root         = root
...
#+BEGIN_SRC python
  ...
  def objective_function(G, clusters, is_terminal, params):
      # DSLAMs cost
      sub_root_cost = len(set(clusters)) * params['c_r']

      vertices = np.arrray([int(v) for v in G.vertices()])
      ...
      for index, cluster_id in enumerate(set(clusters)):
          ...
          best_road_length, best_cable_length, _, _  = get_min_path_tree(G, cluster_nodes, is_terminal, params)

          total_road_length += best_road_length
          total_cable_length = best_cable_length
  ...
  def heuristic_solve(G, params):
      ...
      vertex_dist = get_vertices_distance(G)
      terminal_dist = vertex_dist[is_terminal, is_terminal]
