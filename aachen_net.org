...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  # update other cluster distances from new one, created merging c1 and c2
  ...
  clusters = { int(t): Cluster({ int(t) }) for t in terminals if G.vertex_properties['n_lines'][v] > 0 }
  ...
  while edges:
      ...
      if mergeable(max_cluster_distances, c1_id, c2_id):
          ...
          # update map information for all the nodes in the old one
