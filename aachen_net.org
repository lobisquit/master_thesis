...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  def objective_function(G, clusters, terminals, terminal_dist):
      ...
      for element in set(clusters):
          ...
          cluster_cable_length = np.min(np.sum(inter_cluster_dist, axis=0))
      ...
      for cluster_id in set(clusters):
          # print(cluster_id, end='\r')
          cluster_nodes = clusters[clusters == cluster_id]
          subgraph = GraphView(G, vfilt=lambda v: v in cluster_nodes)

          MST_edges = min_spanning_tree(subgraph,
                                        weights=subgraph.edge_properties['length'])
          total_road_length += sum( subgraph.ep['length'][e]
  ...
  print("Pruning: left", total_couples, "out of", T**2)

  while True:
      print("Money", objective_function(G, clusters, terminals, terminal_dist), "€")
      ...
      if total_n_lines <= params['n_M'] and \
         joint_diameter <= 2 * params['d_M']:
          ...
          print("Joined! n_cluster", len(set(clusters)), end='\r')
