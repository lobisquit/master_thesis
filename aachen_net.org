...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/13_ILP.py :results silent
  ...
  cplex_engine.parameters.workmem.set(2048)
  ...
  for i, (source, target, data) in enumerate(G.edges(data=True)):
      ...
      # 7)
      if source != 'r' and source < target:
          m.add_constraint(ct=X[source][target] + X[target][source] <= 1,
                           ctname="{}".format(name(source, target, var='simple_loops')))
  ...
  with open(graph_path + "_ILP.json", 'w') as output:
      output.write(json.dumps(json_graph.node_link_data(G)))
#+END_SRC
...
#+BEGIN_SRC python :noweb yes :var graph_path=flatten(graph_path) :tangle scripts/aachen_net/14_plot_ILP_result.py
  ...
  with open(graph_path + "_ILP.json") as f:
      js_graph = json.load(f)
  ...
  print("G contains", sum(nodes_df['is_subroot']), "subroots nodes")
...
#+BEGIN_SRC python
  def get_terminal_distance(G, is_terminal, path=None, clear_cache=False):
      if not path:
          distance_path = Path('data/aachen_net/distance_matrix_temp.h5')

      # this takes some time: use cache file
      if not distance_path.exists() or clear_cache:
          vertex_dist = shortest_distance(G, weights=G.edge_properties['length'])

          with h5py.File(distance_path, 'w') as f:
              # obtain which and how many nodes have to be considered terminals
              terminals = list(G.vertices())[is_terminal]
              T = len(terminals)

              # fill matrix with their pairwise distances
              matrix = f.create_dataset('terminal_dist', (T, T))
              for index, vertex_id in enumerate(terminals):
                  if index % 100 == 0:
                      print("Filling distance matrix", index, "/", T, end='\r')

                  matrix[index, :] = np.array(vertex_dist[vertex_id])[is_terminal]

      with h5py.File(distance_path, 'r') as f:
          # keep track of distance among vertices
          terminal_dist = np.array(f['terminal_dist'])

      # check simmetry
      assert np.allclose(terminal_dist, terminal_dist.T, equal_nan=True), \
          "Distance matrix not symmetric"

      return terminal_dist
#+END_SRC

#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  <<imports_&_defaults>>
  <<projection>>
  <<graph_utils>>
  <<terminal_dist_cache>>
  ...
  # load graph and convert to graph_tool format (Floyd-Warshall is waaaay faster)
  if choose_graph == "dummy":
      path = dummy_graph_path
  else:
      path = graph_path + "_complete.json"
  ...
  terminal_dist = get_terminal_distance(G, is_terminal)
  ...
  def check_consistency(clusters, min_cluster_distance):
      for element in set(clusters):
          cluster_idx = np.where(clusters == element)
          cluster_submatrix = min_cluster_dist[cluster_idx, :]

          # check rows are equal (ignore nan)
          assert np.isclose(cluster_submatrix, cluster_submatrix[0], equal_nan=True).all(), \
              "Elements of cluster do not share min distances"

          # check that rows and columns are the same
          assert np.allclose(min_cluster_dist[cluster_idx, :], min_cluster_dist[:, cluster_idx].T, equal_nan=True), \
              "Distance matrix not symmetric"

  def objective_function(G, clusters, terminals, terminal_dist):
      # DSLAMs cost
      sub_root_cost = len(set(clusters)) * params['c_r']

      # to have fiber cable cost, build the shortest minimum path tree for each
      # of the clusters
      total_cable_length = 0
      for element in set(clusters):
          # compute the total length of cable to each j from i, given its number
          # of requested lines
          cluster_vertex = np.where(clusters == element)[0]
          inter_cluster_dist = terminal_dist[cluster_vertex, cluster_vertex]
          cluster_cable_length = inter_cluster_dist.dot(terminal_lines[cluster_vertex])

          # choose the node with minimum cumulative distance from the others
          # to be the center of its group
          cluster_cable_length = np.min(np.sum(cluster_cable_length, axis=0))
          total_cable_length += cluster_cable_length

      cable_cost = total_cable_length * params['c_f']

      # excavation cost
      # get the MST for each cluster, then sum their edges length
      total_road_length = 0
      for cluster_id in set(clusters):
          cluster_nodes = clusters[clusters == cluster_id]
          ...
          if len(cluster_nodes) == 2:
              i, j = np.where(clusters == cluster_id)[0]
              total_road_length += terminal_dist[i][j]
              continue

          if len(cluster_nodes) == 3:
              i, j, k = np.where(clusters == cluster_id)[0]
              A = terminal_dist[i][j]
              B = terminal_dist[j][k]
              C = terminal_dist[k][i]

              total_road_length += A + B + C - min(A, B, C)
              continue

          # create a mask to select this cluster vertices
          mask = np.zeros(N, dtype=bool)
          mask[cluster_nodes] = True

          subgraph = GraphView(G, vfilt=mask)

          # compute the minimum spanning tree
          MST_edges = min_spanning_tree(subgraph, weights=subgraph.ep['length'])
          total_road_length += subgraph.ep['length'].a.dot(MST_edges.a)

      excavation_cost = params['c_e'] * total_road_length

      return sub_root_cost + cable_cost + excavation_cost

  # measure if the connection is more expensive than a DSLAM:
  # in this cases don't even consider evaluating the merge
  critical_length = params['c_r'] / params['c_e']

  logger.info("critical_length = {}".format(critical_length))

  # prune the nodes that even at the beginning are further from each other more
  # than the critical length
  min_cluster_dist[min_cluster_dist > critical_length] = np.nan
  max_cluster_dist[min_cluster_dist > critical_length] = np.nan
  ...
  # prepare plots if in debug mode
  if __debug__:
      plt.ion()
      fig_lines = plt.figure("n° lines per cluster histogram")
      fig_nodes = plt.figure("n° nodes per cluster histogram")

  while True:
      n_iter += 1

      # get two closest clusters (heuristic measure)
      # TODO check if a condition on diameter (max_dist) gives better results
      ...
      if total_n_lines <= params['n_M'] and \
         joint_diameter <= 2 * params['d_M']:
          ...
          print("{}n_cluster {}".format(" "*25, len(set(clusters))), end='\r')
      ...
      if n_iter % 50 == 0:
          total_cost = objective_function(G, clusters, terminals, terminal_dist)
          print("Money {:.4f}M€ {} n_rejected".format(total_cost/1e6, " " * 50))

          if __debug__:
              total_cost = objective_function(G, clusters, terminals, terminal_dist)
              print("Money {:.4f}M€ {} n_rejected {}".format(total_cost/1e6, " " * 50, n_rejected))
              n_rejected = 0

              # check each cluster members have all same rows and same columns in matrix
              check_consistency(clusters, min_cluster_dist)

              fig_lines.clf()
              fig_lines.gca().hist(cluster_lines, bins=100)

              fig_nodes.clf()
              fig_nodes.gca().hist([np.count_nonzero(clusters == element) for element in set(clusters)], bins=100)

              plt.pause(0.1)
              plt.draw()

      # assess if is time to stop
      if np.all(np.isnan(min_cluster_dist)):
          logger.info("Checked all possible couples: finishing")

          # vertices not in a cluster are assigned to cluster -1
          cluster_ids = - np.ones(N)
          cluster_ids[is_terminal] = clusters

          # save information in the graph
          cluster_property = G.new_vertex_property("int")
          cluster_property.a = cluster_ids

          G.vp['cluster_id'] = cluster_property

          G.save(graph_path + "_heuristic_clusters.gt")

          break
#+END_SRC

#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/16_plot_heuristic.py :results silent
  <<imports_&_defaults>>
  <<projection>>
  <<graph_utils>>
  <<terminal_dist_cache>>
  ...
  G = load_graph(graph_path + "_heuristic_clusters.gt")
  clusters = G.vp['cluster_id'].a

  # collect nodes and edges shapes here
  nodes_dfs = []
  edges_dfs = []

  for index, cluster_id in enumerate(set(clusters)):
      print(index, "/", len(clusters), end='\r')

      # ignore non-assigned nodes
      if cluster_id == -1:
          continue

      # compute the minimum spanning tree
      # NOTE this is just for visualization: final configuration will be similar,
      # but not actually the same to this one, since cable cost is not considered
      # here
      subgraph = GraphView(G, vfilt=(clusters == cluster_id))
      MST_edges = min_spanning_tree(subgraph, weights=subgraph.ep['length'])

      nodes_df, edges_df = graph_tool_to_geopandas(subgraph, projection=projection)

      nodes_dfs.append(nodes_df)
      edges_dfs.append(edges_df)

  active_nodes_df = pd.concat(nodes_dfs)
  active_edges_df = pd.concat(edges_dfs)

  fig = plt.figure(figsize=(6, 6), frameon=False)
  ax = fig.gca()
  plot_geopandas_graph(active_nodes_df, active_edges_df, title="Detail of heuristic solution", ax=ax)

  _, all_edges_df = graph_tool_to_geopandas(G, projection=projection)
  all_edges_df.plot(color='#b2b2b2', zorder=-1, ax=ax)
  ...
  plt.savefig('figures/aachen_heuristic_DSLAMs.png', dpi=250)
  plt.close('all')
#+END_SRC
