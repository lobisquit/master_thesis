...
#+BEGIN_SRC python :noweb yes :var choose_graph=flatten(choose_graph) :var graph_path=flatten(graph_path) :var dummy_graph_path=flatten(dummy_graph_path) :var model_params=model_params :tangle scripts/aachen_net/15_heuristic.py :results silent
  ...
  clusters = {
      int(t): Cluster({ int(t) }) for t in terminals
  ...
  def update_distances(max_cluster_distances, clusters, c1_id, c2_id):
      ''' Update other cluster distances from new one, the sum of c1 and c2 '''
      c_new_id = min(c1_id, c2_id)

      for c_id in clusters:
          if c != c1 and c != c2:
              max_cluster_distances[order(c_id, c_new_id)] = max(
                  max_cluster_distances[order(c, c1)],
                  max_cluster_distances[order(c, c2)]
              )

              if debug_:
                  # just to be sure, make other NaN
                  max_cluster_distances[order(c_id, c_new_id, False)] = float('nan')

              if debug_:
                  max_dist = compute_max_distance(clusters, vertex_distances, c_id, c_new_id)
                  assert max_dist == max_cluster_distances[order(c_id, c_new_id)], \
                      'Invariant on max_cluster_distances not guaranteed'

  def mergeable(max_cluster_distances, c1_id, c2_id, n_M=50, d_M=1500):
      if max_cluster_distances[order(c1_id, c2_id)] <= d_M:
          if c1.n_lines + c2.n_lines <= n_M:
              return True
  ...
  while edges:
      ...
      c1_id = node_cluster_map[source]
      c2_id = node_cluster_map[target]
      ...
      if mergeable(max_cluster_distances, c1_id, c2_id):
          c_new_id = min(c1_id, c2_id)
          c_old_id = max(c1_id, c2_id)

          # update map information for all the nodes in the old cluster (the one to delete)
          for vertex in clusters[c_old_id]:
              node_cluster_map[vertex] = c_new

          # merge the two candidates
          clusters[c_new_id] += clusters[c_old_id]

          # update distance information for all other clusters
          update_distances(max_cluster_distances, clusters, c_new_id, c_old_id)

          # delete cluster
          del clusters[c_old_id]
