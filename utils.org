* Imports and defaults settings
  ...
  #+BEGIN_SRC python
    ...
    logger.debug('matplotlib ok')
  #+END_SRC

  Useful function to flatten org tables in ~python~ lists of strings or single string.
  #+NAME: org_flatten
  #+BEGIN_SRC python
    def org_flatten(org_variable):
        flat_list = list(np.array(org_variable).flatten())
