...
** Match each building with the closest road
   ...
   #+BEGIN_SRC python :noweb yes :tangle scripts/citymap/7_get_closest_roads.py :var valid_types=utils.org:valid_types :var closest_lines_path=closest_lines_path :var graph_path=graph_path
     ...
     closest_lines.to_csv(closest_lines_path[0][0], quoting=csv.QUOTE_NONNUMERIC, index=None)
   #+END_SRC
...
** Project each building on its road
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=utils.org:valid_types :var graph_path=graph_path :tangle scripts/citymap/8_add_buildings_to_graph.py :var closest_lines_path=closest_lines_path
     ...
     closest_lines = pd.read_csv(closest_lines_path[0][0])

     def process_edge(group):
         edge = eval(group.edge.iloc[0]) # load tuple
         ...
         for _, row in group.iterrows():
             new_data.append({
                 'osm_id': row.osm_id,
                 'new_point': wkt.loads(row.new_point),
         ...
         for point in new_points:
             G.add_node(n=point['osm_id'],
                        lon=point['new_point'].x,
                        lat=point['new_point'].y,
         ...
         # add new ones
         G.add_edge(edge[0], new_points[0])
         ...
         G.add_edge(new_points[-1], edge[1])

     closest_lines.groupby('edge').apply(process_edge)

     with open(graph_path[0][0].replace('.json', '_with_buildings.json'), 'w') as output:
         output.write(json.dumps(G))
   #+END_SRC
