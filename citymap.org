...
** Create an abstract graph from the roads
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=utils.org:valid_types :tangle scripts/citymap/7_get_roads_graph.py
     <<imports_&_defaults>>
     <<roads>>
     <<buildings>>
...
** Project each building on its road
   ...
   #+BEGIN_SRC python :noweb yes :var valid_types=utils.org:valid_types
     <<imports_&_defaults>>
     <<roads>>
     <<buildings>>

     closest_roads = pd.read_csv('data/citymap/closest_roads.csv')
